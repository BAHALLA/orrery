...
- **Plugins over per-agent callbacks**: Cross-cutting concerns (RBAC, guardrails, autonomy, metrics, audit, activity tracking, resilience, output capping, error handling, prompt-injection screening, PII redaction) are packaged as ADK `BasePlugin` subclasses in `core/orrery_core/plugins/` and registered once on the `Runner` via `default_plugins()`. Plugins apply globally to every agent, tool, and LLM call. Order matters: `IdentityStateGuardPlugin` is registered **first of all** (it snapshots the identity/authorization keys before every tool call and reverts any change the call made, except clearing a consumed confirmation decision; the `_role_set_by_server`/`_autonomy_set_by_server` locks are plain booleans in the same state, so without it they would protect only as long as no tool ever wrote a state key it shouldn't); `AuditPlugin` is registered **before** the gates (ADK's before-tool chain early-exits on the first non-None return, so audit records the attempt even when a gate denies the call), `AutonomyPlugin` before `GuardrailsPlugin` (the level is a property of the process, so a tool it forbids is refused rather than queued for an approval that cannot help it), and `ToolOutputCapPlugin` runs last among after-tool observers (it returns a replacement for oversized results, which would early-exit the after chain).
